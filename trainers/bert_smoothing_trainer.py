...
class BertSmoothing_Trainer(Trainer):
    def __init__(self, args, logger, log_dir, model_config, full_dataset, random_state):
        super(BertSmoothing_Trainer, self).__init__(args, logger, log_dir, model_config, full_dataset, random_state)
        self.smoothing_factor = args.smoothing_factor

    def train(self, args, logger, full_dataset):
        logger.info('[LabelSmoothing] Bert Smoothing Trainer: training started')
        ...
        # train the network
        for idx in tqdm(range(num_training_steps), desc=f'[LabelSmoothing] Training...'):
            ce_loss_mean = 0.0
            ...
            for i in range(args.gradient_accumulation_steps):
                ...
                nll_loss = \
                    self.forward_backward_vanilla_batch(model, {'nl_batch': nl_batch}, loss_fn,  args, device)
    ...
    def forward_backward_vanilla_batch(self, model, data_dict, loss_fn, args, device):
        ...
        loss = loss_fn(outputs, n_labels, smoothing=self.smoothing_factor)
